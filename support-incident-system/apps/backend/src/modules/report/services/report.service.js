...
const { formatDateForEmail, formatDateOnly, calcResolutionDuration } = require("../../../common/utils/date");
const { getTicketTitle, formatAssigneeForWeb, formatEvidenceForDisplay } = require("../../../common/utils/ticket");
...
async function generateReport(ticketId, reportType = "STANDARD", generatedBy = "System") {
  ...
  // FIX: Gunakan direct prisma.$queryRaw (ReportRepo tidak diimport di file ini)
  // Sama dengan implementasi V13 original
  const insertResult = await prisma.$queryRaw`
    ...
    VALUES
      (${ticket.id}, ${reportType}, ${reportTitle}, ${JSON.stringify(reportContent)}::jsonb,
       ${htmlFilePath}, ${htmlFileName}, ${generatedBy})
    RETURNING id
  `;
  const dbReportId = insertResult[0]?.id ? Number(insertResult[0].id) : null;
