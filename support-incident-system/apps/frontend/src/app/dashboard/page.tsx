...
function resolveAssignee(raw: any): string {
  ...
  if (Array.isArray(raw)) {
    ...
    const names = raw.map((a) => {
      ...
      if (typeof a === "object" && a !== null) return (a.username || a.name || a.displayName || "").trim();
      return "";
    }).filter(Boolean);
    return names.length > 0 ? names.join(", ") : "—";
  }
  if (typeof raw === "string") {
    try { const parsed = JSON.parse(raw); return resolveAssignee(parsed); }
    catch { return raw.trim() || "—"; }
  }
  if (typeof raw === "object") return (raw.username || raw.name || raw.displayName || "—").trim();
  return "—";
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function resolveReporter(t: any): string {
...
export default async function DashboardPage() {
  ...
  const mapTicket = (t: any) => ({
    ...
    reporter:        resolveReporter(t),
  });

  const allTicketData = allTickets.map(mapTicket);
  const incidentData = incidents.map(mapTicket);
