...
const config = {
  ...
  portal: {
    ...
    // MONOREPO: URL Next.js frontend (untuk redirect & startup banner)
    frontendUrl:    process.env.FRONTEND_URL || `http://localhost:${process.env.FRONTEND_PORT || 3001}`,
    // Self-call URL: "backend" = service name di docker-compose (sis-network)
    // Dev: http://localhost:3000 | Docker: http://backend:3000
    backendSelfUrl: process.env.BACKEND_SELF_URL || null,
  },

  // Formbricks Form IDs
  // Dibaca dari env agar bisa di-override via docker-compose / .env
  // docker-compose: FORM_ID_TICKETING & FORM_ID_INCIDENT
  formbricks: {
