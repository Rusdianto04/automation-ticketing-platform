 ...
 *     → Jalankan: docker compose restart sis-backend
 *
...
function getPublicUrl() {
  ...
  // Gunakan FRONTEND_PORT (3001) bukan backend port (3000)
  // karena URL yang dikirim ke user harus menuju Next.js portal, bukan Express API
  ...
  if (portalUrl && isValidUrl(portalUrl)) {
    // Jika PORTAL_URL sudah set manual, pastikan pakai port 3001
    // Ganti port 3000 → 3001 jika user salah isi
    _cachedUrl = portalUrl.replace(/:3001(\/|$)/, `:${frontendPort}$1`).replace(/\/$/, "");
    console.log(`🌐 [NETWORK] Portal URL (PORTAL_URL env): ${_cachedUrl}`);
    return _cachedUrl;
  }

  // ── Prioritas 2: HOST_IP dari .env ────────────────────────────────────────
  const hostIp = (process.env.HOST_IP || config.portal?.hostIp || "").trim();
  if (hostIp && hostIp !== "localhost" && hostIp !== "127.0.0.1") {
    _cachedUrl = `http://${hostIp}:${frontendPort}`;
    console.log(`🌐 [NETWORK] Portal URL (HOST_IP env): ${_cachedUrl}`);
    return _cachedUrl;
  }

  // ── Prioritas 3: /proc/net/fib_trie — IP host fisik ─────────────────────
  const fibIp = getHostIpFromFibTrie();
  if (fibIp) {
    _cachedUrl = `http://${fibIp}:${frontendPort}`;
    console.log(`🌐 [NETWORK] Portal URL (fib_trie): ${_cachedUrl}`);
    return _cachedUrl;
  }

  // ── Prioritas 4: Gateway dari /proc/net/route ─────────────────────────────
  const gatewayIp = getDockerHostIp();
  if (gatewayIp && gatewayIp !== "172.17.0.1") {
    _cachedUrl = `http://${gatewayIp}:${frontendPort}`;
    console.log(`🌐 [NETWORK] Portal URL (docker gateway): ${_cachedUrl}`);
    return _cachedUrl;
  }

  // ── Prioritas 5: LAN IP dari os.networkInterfaces ─────────────────────────
  const lanIp = detectLanIp();
  if (lanIp) {
    _cachedUrl = `http://${lanIp}:${frontendPort}`;
    console.log(`🌐 [NETWORK] Portal URL (networkInterfaces): ${_cachedUrl}`);
    return _cachedUrl;
  }

  // ── Fallback terakhir ─────────────────────────────────────────────────────
  _cachedUrl = `http://localhost:${frontendPort}`;
  console.log(`🌐 [NETWORK] Portal URL (fallback localhost): ${_cachedUrl}`);
  return _cachedUrl;
}
