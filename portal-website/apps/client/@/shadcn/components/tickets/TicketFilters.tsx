...
type FilterType = "status" | "assignee" | null;
...
export default function TicketFilters({
  ...
  const assignees = useMemo(() => {
    const baseAssignees = ["Unassigned"];
    if (!users || !Array.isArray(users)) return baseAssignees;
    return [...baseAssignees, ...users.map(u => u?.name || "Unknown")];
  }, [users]);
    ...
    <div className="flex flex-row items-center gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        {selectedStatuses.map((status) => (
          <FilterBadge key={`status-${status}`} text={`Status: ${status}`} onRemove={() => onStatusToggle(status)} />
        ))}
        {selectedAssignees.map((assignee) => (
          <FilterBadge key={`assignee-${assignee}`} text={`Assignee: ${assignee}`} onRemove={() => onAssigneeToggle(assignee)} />
        ))}
