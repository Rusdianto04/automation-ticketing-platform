                    ...
                    {uiSettings.showAvatars && (
                      <div className="flex flex-row space-x-1">
                        {ticket.engineers && ticket.engineers.length > 0
                          ? ticket.engineers.map((eng: any) => (
                            <span key={eng.id} className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-500" title={eng.name}>
                              <span className="text-[11px] font-medium leading-none text-white uppercase">{eng.name[0]}</span>
                            </span>
                          ))
                          : ticket.assignedTo && (
                            <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-500">
                              <span className="text-[11px] font-medium leading-none text-white uppercase">{ticket.assignedTo.name[0]}</span>
                            </span>
                          )}
                      </div>
                    )}
