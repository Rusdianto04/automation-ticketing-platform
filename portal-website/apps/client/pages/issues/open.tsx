...
export default function OpenIssues() {
  ...
  async function fetchTickets() {
    try {
      const response = await fetch(`/api/v1/tickets/open`, {
        method: "GET",
    ...
    <div className="flex flex-col p-4 sm:p-8 w-full bg-background min-h-screen">
      <div className="w-full max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Open Issues</h1>
          <TicketFilters
        ...
        {filteredTickets.length === 0 ? (
          <div className="text-center py-12 bg-card rounded-lg border">
            <p className="text-muted-foreground text-lg">No open tickets found</p>
          </div>
        ) : (
                  ...
                  {filteredTickets.map((ticket: any) => (
                    ...
                    >
                      ...
                      <td className="hidden px-3 py-4 text-sm sm:table-cell">
                        <span className="inline-flex items-center gap-x-1.5 rounded-md bg-green-100 dark:bg-green-950 px-2 py-1 text-xs font-medium text-green-700 dark:text-green-300">
